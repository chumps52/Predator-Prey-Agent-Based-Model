...
mig=0;
cnt=1;
dir=rand*2*pi;              %human has been unable to kill a lynx, so chooses a random direction to move in
while mig==0&cnt<=8        %human has up to 8 attempts to migrate (without leaving the edge of the model)
    ...
    if npos(1)<ENV_DATA.bm_size&npos(2)<ENV_DATA.bm_size&npos(1)>=1&npos(2)>=1   %check that human has not left edge of model - correct if so.
       mig=1;
    end
    cnt=cnt+1;
