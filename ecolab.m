function ecolab(size,nr,nf,nh,nsteps,fmode,outImages)
...
%create_environment.m)
    ...
    [agent]=create_agents(nr,nf,nh);       %create nr rabbit and nf fox agents and places them in a cell array called 'agents'
    create_messages(nr,nf,nh,agent);       %sets up the initial message lists
    initialise_results(nr,nf,nh,nsteps);   %initilaises structure for storing results
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %MODEL EXECUTION
    for n_it=1:nsteps                   %the main execution loop
        N_IT=n_it;
        if mod(N_IT,4)==0
            grass_regrowth();  %every 4 weeks the grass will regrow by a bit
        end
        ...
        if fmode == true                                       % if fastmode is used ...
           for test_l=1 : 5                                    % this checks the total number agents and adjusts the CONTROL_DATA.fmode_display_every variable accoringly to help prevent extreme slowdown
            ...
            if IT_STATS.tot_r(n_it) == 0             %fastmode convergence - all lynxes eaten - all lynxes will now die
                disp('Fast mode convergence criteria satisfied - no hares left alive! > ')
                break
            end  
            if IT_STATS.tot_f(n_it) == 0             %fastmode convergence - all hares starved - hares will now proliferate unchecked until all vegitation is eaten
                disp('Fast mode convergence criteria satisfied - no lynxes left alive ! > ')
                break
            end
        end
    end
