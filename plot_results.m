function plot_results(agent,nsteps,fmode,outImages)
    ...
    global N_IT IT_STATS ENV_DATA MESSAGES CONTROL_DATA PARAM
    ...
    disp(strcat('Human kills = ',num2str(nk)))
    disp(strcat('No. new hares = ',num2str(IT_STATS.div_r(N_IT+1))))
    disp(strcat('No. new lynx = ',num2str(IT_STATS.div_f(N_IT+1))))
    disp(strcat('No. agents migrating = ',num2str(IT_STATS.mig(N_IT+1))))
    disp(strcat('No. hares dying = ',num2str(IT_STATS.died_r(N_IT+1))))
    disp(strcat('No. lynx dying = ',num2str(IT_STATS.died_f(N_IT+1))))
    disp(strcat('No. hares eaten = ',num2str(IT_STATS.eaten(N_IT+1))))
    ...
    if (fmode==false) || (N_IT==nsteps) || ((fmode==true) && (rem(N_IT , CONTROL_DATA.fmode_display_every)==0))
        ...
        col{1}='r-';                   %set up colours that will represent different cell types red for rabbits, blue for lynx
        col{2}='b-';
        ...
        subplot(2,1,1),cla
        subplot(2,1,1),plot((1:N_IT+1),nr(1:N_IT+1),col{1});
        subplot(2,1,1),axis([0 nsteps 0 1.1*max(nr)]);
        subplot(2,1,2),cla
        subplot(2,1,2),plot((1:N_IT+1),nf(1:N_IT+1),col{2});
        subplot(2,1,2),axis([0 nsteps 0 1.1*max(nf)]);
        subplot(2,1,1),title('Number of Hares alive');
        subplot(2,1,2),title('Number of Lynxes alive');
        ...
        for cn=1:length(agent)                          %cycle through each agent in turn
            if typ(cn)>0                                %only plot live agents
                ...
                if isa(agent{cn},'hare')              %choose plot colour depending on agent type
