...
%SUMMARY OF HARE MIGRATE RULE
%Hares will migrate only if they have not eaten
%Hares will always try to migrate towards the nearest food source
%The Hares will extract the distibution of food in its LOCAL environment (at
...
mig=0;                               %indicates whether hares has successfully migrated
pos=agt.pos;                         %extract current position 
cpos=round(pos);                     %round up position to nearest grid point   
spd=agt.speed;                       %hares migration speed in units per iteration - this is equal to the food search radius
...
mig=0;                          %flag will be reset to one if hare migrates
...
if mig==0                                   %hare has been unable to find food, so chooses a random direction to move in      
    cnt=1;
    dir=rand*2*pi;              
    while mig==0&cnt<=8                     
        npos(1)=pos(1)+spd*cos(dir);        %new x co-ordinate
        npos(2)=pos(2)+spd*sin(dir);        %new y co-ordinate
        if npos(1)<ENV_DATA.bm_size&npos(2)<ENV_DATA.bm_size&npos(1)>=1&npos(2)>=1   %check that lynx has not left edge of model - correct if so.
           mig=1;
        end
        cnt=cnt+1;
