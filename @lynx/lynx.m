classdef lynx           %declares lynx object
    ...
    methods                         %note that this class definition mfile contains only the constructor method!
        ...
        function f=lynx(varargin) %constructor method for fox  - assigns values to fox properties
                %f=fox(age,food,pos....)
                %
            ...
            switch nargin                     %Use switch statement with nargin,varargin contructs to overload constructor methods
                ...
                case 1                         %input is already a fox, so just return!
                    if (isa(varargin{1},'lynx'))		
                        f=varargin{1};
                    else
                        error('Input argument is not a lynx')
                    end
                ...
                otherwise
                    error('Invalid no. of input arguments for lynx')
            end
        end
    end
end